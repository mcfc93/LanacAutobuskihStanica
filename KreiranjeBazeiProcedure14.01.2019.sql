...
CREATE TABLE IF NOT EXISTS `bus`.`popust_prevoznika` (
  ...
  `DjackiPoput` DOUBLE NULL DEFAULT '1',
...
CREATE TABLE IF NOT EXISTS `bus`.`prevoznik` (
  ...
  `PostanskiBroj` INT(11) NULL DEFAULT NULL,
  `Stanje` SET('Aktivno', 'Blokirano', 'Izbrisano') NULL DEFAULT 'Aktivno',
  `IdPopusta` INT(11) NULL DEFAULT NULL,
  PRIMARY KEY (`JIBPrevoznika`),
  INDEX `PrevoznikMjesto_fk_idx` (`PostanskiBroj` ASC) VISIBLE,
  INDEX `Popust_FK_idx` (`IdPopusta` ASC) VISIBLE,
  CONSTRAINT `Popust_FK`
    FOREIGN KEY (`IdPopusta`)
    REFERENCES `bus`.`popust_prevoznika` (`IdPopust`)
    ON UPDATE CASCADE,
...
begin 
	...
	update nalog, zaposleni set nalog.Stanje=pStanje, zaposleni.Stanje=pStanje where nalog.JIBStanice=pJIBStanice and nalog.JMBG=zaposleni.JMBG;
end$$
